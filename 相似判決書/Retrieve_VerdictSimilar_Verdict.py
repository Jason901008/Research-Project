...
verdict_file = './Verdict_Data/Injure/Negligent_Injury_original_data.csv'
all_result_file = './Verdict_Data/Injure/Negligent_Injury_result.csv'
val_num_file = './Verdict_Data/Injure/val_number.csv'
ner_file = './Verdict_Data/Injure/Negligent_Injury_predict_ner.pkl'
...
#------------------------------------------------------------------------------
folder_path = './MODEL/Drug'

ner_model = CkipNerChunker(model_name=folder_path)
...
def tfidf(classifier , number):
    sim_list = []
    sim_folder = './sim_data'
    sim_data_paths = [os.path.join(sim_folder, filename) for filename in os.listdir(sim_folder)]
    
    for sim_path in sim_data_paths:
        df_sim = pd.read_csv(sim_path, encoding='utf-8-sig', header=None)
        sim_list.append(df_sim[0])

    folder = './interface_train'
    ...
    #取前N筆
    N = 10
    sorted_values = sorted(result_cos, reverse=True)
    ...
    tfidf_text_df.to_csv(f'{classifier}_word_sim_tf_{number}.csv', index=False, header=False , encoding='utf-8-sig')
...
def embedding(classifier , number):
    ...
    word_df.to_csv(f'{classifier}_word_sim_embedding_{number}.csv', index=False, encoding='utf-8-sig')   
